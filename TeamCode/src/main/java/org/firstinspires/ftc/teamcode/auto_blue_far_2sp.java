...
public class auto_blue_far_2sp extends OpMode { // requires human player
    ...
    private final Pose intake3CP = new Pose(61.113, 73.57); // Control Point
    private final Pose intake3End = new Pose(14.59, 56.98, Math.toRadians(182)); // Lowest (Third Set) of Artifacts from the Spike Mark
    private final Pose park = new Pose(30.153, 82.491, Math.toRadians(180)); // Park in front of gate at end of auto
    private PathChain startToScore, scoreToIntake1, intake1ToScore, scoreToIntake2SetUp,
            intake2SetUpToEnd, intake2ToScore, scoreToIntake3, intake3ToScore, scoreToPark;
    ...
    public void buildPaths(double power_adj) {
        ...
        scoreToIntake2SetUp = follower.pathBuilder()
                .addPath(new BezierCurve(score, intake2SetUpCP, intake2SetUpEnd))
                .setLinearHeadingInterpolation(score.getHeading(), intake2End.getHeading(), 0.6)
                .addParametricCallback(0.3, () -> Intake.setPower(ftc_fns.ball_pickup_intake_pwr * power_adj))
                .build();
        intake2SetUpToEnd = follower.pathBuilder()
                .addPath(new BezierLine(intake2SetUpEnd, intake2End))
                .setLinearHeadingInterpolation(score.getHeading(), intake2End.getHeading(), 0.6)
                .build();
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 2:
                ...
                break;
            case 3:
                shootThenFollowPath(0, scoreToIntake2SetUp, 1, 4);
                break;
            case 4:
                waitThenFollowPath(0, intake2SetUpToEnd, 1, 5);
                break;
            case 5:
